...
from app.schemas.incident import (
    ...
    IncidentListResponse, TimelineEntryCreate
)
...
async def update_incident(
    ...
    # Calculer MTTR si résolution
    if payload.status == IncidentStatus.RESOLVED and old_status != IncidentStatus.RESOLVED:
        ...
        entry = IncidentTimeline(
            incident_id=incident.id,
            event_type="resolved",
            message=f"Incident résolu par {current_user.full_name or current_user.username}",
            author_id=current_user.id,
            author_name=current_user.full_name or current_user.username,
        )
        db.add(entry)
